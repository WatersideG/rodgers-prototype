...
import os as _os
# Body typeface. Switch with RSX_BODY_FONT=source-serif (or add a key). Default stays Spectral.
BODY_FONTS = {
  "spectral":     ('"Spectral",Georgia,"Times New Roman",serif',      'Spectral:ital,wght@0,400;0,600;0,700;1,400'),
  "source-serif": ('"Source Serif 4",Georgia,"Times New Roman",serif','Source+Serif+4:ital,opsz,wght@0,8..60,400;0,8..60,600;0,8..60,700;1,8..60,400'),
}
BODY_FONT = BODY_FONTS[_os.environ.get("RSX_BODY_FONT","spectral")]

CSS = r"""
:root{
  --navy:#012D5D; --navy2:#0A2342; --deep:#071B33; --ink:#16283F; --body:#33475E;
  --ice:#EDF1F7; --ice2:#E2E9F2; --cream:#F7F4EE; --steel:#6B85A1; --line:#D5DEE9; --accent:#E8722C; --accent2:#C85E20;
  --ease:cubic-bezier(.4,0,.2,1); --dur:.2s;
}
*{box-sizing:border-box;margin:0;padding:0}
html{scroll-behavior:smooth}
body{font-family:BODYFONTSTACK;color:var(--body);background:#fff;font-size:16px;line-height:1.6}
...
.btn{display:inline-flex;align-items:center;justify-content:center;min-height:38px;box-sizing:border-box;vertical-align:middle;white-space:nowrap;background:var(--navy);color:#fff;font-size:11px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;padding:9px 16px;border-radius:0;border:2px solid var(--navy);transition:background var(--dur) var(--ease),border-color var(--dur) var(--ease),color var(--dur) var(--ease);cursor:pointer;line-height:1.2;margin:0 8px 8px 0}
.btn:hover{background:var(--navy2);border-color:var(--navy2)}
.btn.cta{background:var(--accent);border-color:var(--accent)}
.btn.cta:hover{background:var(--accent2);border-color:var(--accent2)}
.credband .btn,footer.site .btn,.mobilebar .btn,.onnavy .btn{background:#fff;border-color:#fff;color:var(--navy)}
.credband .btn:hover,footer.site .btn:hover,.mobilebar .btn:hover,.onnavy .btn:hover{background:var(--cream);border-color:var(--cream)}
.credband .btn.cta,footer.site .btn.cta,.mobilebar .btn.cta,.onnavy .btn.cta{background:var(--accent);border-color:var(--accent);color:#fff}
...
.hero .ctas{margin-top:24px;display:flex;flex-wrap:wrap}
.hero .ctas .btn,.hero .quick .btn{margin-bottom:0}
...
.hero .quick{position:absolute;right:32px;bottom:56px;z-index:3;display:flex}
.hero .quick .btn:last-child{margin-right:0}
.hero.short .quick{bottom:48px}
...
.prod span{font-size:12px;color:var(--steel)}
...
.ice{background:var(--cream)}
.ice .card,.ice .pick,.ice .hoursbox,.ice .feature,.ice .wx,.ice .prod a,.ice .prod div{border-color:#E6E0D4}
...
.pricelist th{font-size:11px;letter-spacing:2px;text-transform:uppercase;color:var(--steel);text-align:left;padding:8px 12px}
.pricelist td{padding:11px 12px;color:var(--ink);vertical-align:top}
...
.timeline b{display:block;font-size:24px;font-weight:700;color:var(--navy);line-height:1;margin-bottom:6px}
.timeline p{font-size:13.5px}
/* seasonal cards */
.season .card.img .ph{aspect-ratio:4/3}
.card.partner .ph{aspect-ratio:4/3;position:relative}.card.partner .ph img{width:100%;height:100%;object-fit:cover;display:block}.card.partner .ph .credit{position:absolute;right:8px;bottom:6px;font-size:10px;color:#fff;text-shadow:0 1px 4px rgba(0,0,0,.6);letter-spacing:.5px}
...
.hoursbox .acts{margin-top:auto;padding-top:14px;margin-bottom:-8px}
...
@media (max-width:940px){
  ...
  .g3,.g4,.g5,.g6,.prod{grid-template-columns:1fr 1fr}
...
@media (max-width:600px){
  ...
  .hero .quick{position:static;margin-top:0;display:grid;grid-template-columns:1fr 1fr;gap:8px}
  .hero .quick .btn{width:100%;margin:0}
  ...
  .hero .ctas .btn{width:100%;margin:0;padding:9px 8px}
...
"""
CSS = CSS.replace("BODYFONTSTACK", BODY_FONT[0])
...
def head(title, desc, canonical):
    return (f'<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
            ...
            f'<link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@500;600;700;800&family={BODY_FONT[1]}&display=swap" rel="stylesheet">'
...
def pricelist(rows, cols=("Service","Price"), note=""):
    ...
    return f'<div class="tscroll"><table class="pricelist"><thead><tr>{th}</tr></thead><tbody>{body}</tbody></table></div>{n}'
